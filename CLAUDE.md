...
3. Cache miss → job goes into `TranscriptionQueue` (always 1 concurrent job, LIFO priority — a newly-opened track jumps ahead of anything still queued). User-initiated jobs (`OnMediaChangedAsync`/`ReanalyzeAsync`) enqueue with `preempt: true`, which cancels whatever job is currently *running* (via a per-running-job linked `CancellationTokenSource`, not the queue's shutdown token) so analysis switches to the newly-opened track immediately instead of waiting for the old one to finish; the preempted job's partial work is discarded (not cached). Background idle-prefetch jobs (`EnsureAnalyzedAsync`) enqueue with `preempt: false` (default) so they never interrupt each other or a user job. The job: `AudioExtractor` (ffmpeg → 16kHz mono WAV, cancellation kills the ffmpeg process via `CancellableThrough(ct)`) → `WhisperTranscriber.TranscribeAsync` (Whisper.net, streams `TranscriptSegment`s via `IAsyncEnumerable`, cancellation observed between segments) → each segment passes `HallucinationFilter` before being pushed to `LyricsViewModel.AddSegment` (streaming display, not wait-for-completion) → on completion, `TranscriptionCache.Save` writes atomically (`.tmp` → `File.Move` with overwrite).
...
`IdleAnalysisService` runs on a timer and, whenever `TranscriptionQueue.IsIdle`, first tries the current playlist's upcoming tracks (in list order, ahead of the whole-library scan), then walks the registered library folders pre-analyzing one not-yet-cached file per tick via `TranscriptionCoordinator.EnsureAnalyzedAsync` (cache-only side effect, never touches the lyrics VM). Any user-initiated `OnMediaChangedAsync` still jumps the queue ahead of these background jobs *and* preempts one if it's already running (see above). A per-session `_visited` set avoids rescanning; `LibraryViewModel.FoldersChanged` resets it.
